...
*   **Karpathy + Hermes Guardrails**：內建「外科手術式修改」與「反幻覺護欄 (Anti-Hallucination)」，明確行為邊界。
*   **模版生成**：支援 **Multi-Phase Workflow** (探索-收集-分析-驗證) 與 **Persona 切換** 邏輯。
...
*   **[NEW] Karpathy Best Practices**：整合「Think Before Coding」、「Simplicity First」等原則的檢查清單。
...
3.  **測試案例 C (Karpathy Verification)**：檢查輸出的 `SKILL.md` 是否包含 `Verification` 循環格式。
    *   預期：步驟應呈現 `1. [動作] → verify: [結果]` 格式。
