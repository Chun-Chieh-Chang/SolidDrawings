...
- **D（實作）**:
  ...
  - 前端: `HeavyEngineClient` 擴充 4 API；`OpenScadPanel`（Editor / Import / Text-to-CAD 三分頁 + Three.js mesh 預覽 + 插入文件）；FeatureManagerPanel 特徵圖示；PartFeaturePropertyManager 新增 rollouts（腳本可再編輯 → rebuild）。
- **C（驗證）**: 後端 pytest 全綠 141 passed / 1 skipped（`test_surface_cut.py` 既有 pythonocc binding 問題，與本 sprint 無關）；OpenSCAD 相關 19 測試含真實 binary（compile → STL → OCCT round-trip）全數通過；前端 `tsc --noEmit` 零新錯誤；E2E 冒煙 `tests/e2e/openscad-smoke.spec.ts` 2 passed（Ribbon → 面板 → 編譯預覽 → 插入特徵 → 視埠顯示）。
