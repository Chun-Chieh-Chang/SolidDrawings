...
export class FilletToolHandler implements SketchToolHandler {
  private state: FilletState = { phase: 'SELECT_EDGE1', edge1Id: null };
  ...
  onPointerDown(ctx: SketchToolContext): void {
    ...
    if (this.state.phase === 'SELECT_EDGE1') {
      ...
      cadState.setHint('Select second edge for fillet');
    } else if (this.state.phase === 'SELECT_EDGE2') {
      if (closest === this.state.edge1Id) return;
      this.performFillet(this.state.edge1Id!, closest);
  ...
  private performFillet(edgeId1: string, edgeId2: string): void {
    const radius = 10; // default radius — override via setFilletRadius()
    const ok = sketchActions.applyFillet(edgeId1, edgeId2, radius);
    if (ok) {
      useCadStore.getState().setHint('Fillet applied');
    } else {
      useCadStore.getState().setHint('Fillet failed — edges may not intersect');
    }
  }
