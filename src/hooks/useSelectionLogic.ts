...
export function useSelectionLogic(deps: SelectionLogicDeps) {
  ...
  useEffect(() => {
    const handleShiftClick = (e: MouseEvent) => {
      ...
      if (e.shiftKey) {
        store.pushToast("Shift+Click: Add range to selection", "info");
      }
      if (e.ctrlKey) {
        store.pushToast("Ctrl+Click: Toggle individual selection", "info");
      }
    };
    document.addEventListener("mousedown", handleShiftClick);
    return () => document.removeEventListener("mousedown", handleShiftClick);
  }, [selectedId]);
}
