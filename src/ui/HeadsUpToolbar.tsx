...
export const HeadsUpToolbar: React.FC = () => {
  ...
  return (
    <div 
      className={`absolute flex items-center gap-0.5 p-0.5 bg-[#F5F6F9] border border-[#D0D0D0] z-40 select-none font-sans ${isDragging ? '' : ''}`}
      style={{
        top: `calc(0.75rem + ${position.y}px)`,
        left: `calc(50% + ${position.x}px)`,
        transform: 'translateX(-50%)'
      }}
    >
      <div 
        className="h-7 flex flex-col flex-wrap gap-[2px] items-center justify-center cursor-move text-slate-400 hover:text-slate-500 px-0.5 border-r border-[#D0D0D0] opacity-60 hover:opacity-100 transition-all touch-none"
      ...
      <button onClick={handleZoomToFit} className="w-7 h-7 flex items-center justify-center rounded-sm hover:bg-slate-200/50 text-slate-600 transition-all border-none bg-transparent cursor-pointer" title="Zoom to Fit (F)">
        <ZoomIcon />
      </button>

      <div className="relative">
        <button onClick={() => setShowOrientation(!showOrientation)} className={`w-7 h-7 flex items-center justify-center rounded-sm transition-all border-none bg-transparent cursor-pointer ${showOrientation ? 'bg-blue-600 text-white' : 'hover:bg-slate-200/50 text-slate-600'}`} title="View Orientation">
          <ViewIcon />
        </button>

        {showOrientation && (
          <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 w-44 bg-white border border-[#C0C0C0] shadow-md py-1 z-50">
            {[
              { id: 'NORMAL_TO', label: 'Normal To', key: 'N', color: 'bg-slate-600' },
              ...
              <button key={v.id} onClick={() => setOrientation(v.id as any)} className="w-full text-left px-3 py-1.5 text-[11px] text-slate-700 hover:bg-[#005B9A] hover:text-white transition-colors flex items-center gap-2 border-none bg-transparent cursor-pointer">
                <span className={`w-4 h-4 ${v.color} text-white rounded flex items-center justify-center text-[8px] font-bold`}>{v.key}</span>
                {v.label}
              </button>
            ))}
          </div>
      ...
      <div className="relative">
        <button
          ...
          className={`w-7 h-7 flex items-center justify-center rounded-sm transition-all border-none cursor-pointer ${
        ...
        {showFilterMenu && (
          <div className="absolute top-full left-0 mt-1 w-32 bg-white border border-[#C0C0C0] shadow-md py-0.5 z-50">
            {FILTER_OPTIONS.map((opt) => (
              <button
                ...
                className={`w-full text-left px-2 py-1 flex items-center gap-1.5 transition-all border-none bg-transparent cursor-pointer ${
      ...
      <button
        ...
        className="w-7 h-7 flex items-center justify-center rounded-sm hover:bg-slate-200/50 text-slate-600 transition-all border-none bg-transparent cursor-pointer"
      ...
      {isSketchMode && (
        <>
          ...
          <button 
            ...
            className="px-2 h-7 flex items-center gap-1 rounded-sm bg-emerald-50 hover:bg-emerald-100 text-emerald-700 transition-all border border-emerald-200/50 cursor-pointer font-medium text-[10px]"
            title="Exit Sketch"
          >
      ...
      {selectionFilter !== 'NONE' && (
        <div className="absolute -bottom-5 left-1/2 -translate-x-1/2 px-1.5 py-0.5 bg-slate-700 text-white text-[9px] rounded-sm whitespace-nowrap z-50">
