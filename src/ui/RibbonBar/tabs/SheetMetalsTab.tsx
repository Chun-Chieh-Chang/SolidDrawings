...
export const SheetMetalsTab: React.FC<RibbonTabProps> = ({
  ...
  return (
    <div className="flex items-center gap-2 h-full animate-in fade-in slide-in-from-left-2 duration-300">
      ...
      <button onClick={() => { if (handleUnfold) { handleUnfold(); } else { setHint('Unfold: selectively flatten a bend.'); } setActiveTab('SHEET_METALS'); }} className="flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group" title="Unfold">
        <div className="w-10 h-10 flex items-center justify-center text-amber-600 transition-transform group-hover:scale-110">
      ...
      <button onClick={() => { if (handleFold) { handleFold([]); } else { setHint('Fold: re-fold a previously unfolded bend.'); } setActiveTab('SHEET_METALS'); }} className="flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group" title="Fold">
        <div className="w-10 h-10 flex items-center justify-center text-amber-600 transition-transform group-hover:scale-110">
      ...
      <button onClick={() => { setShowBendTable(true); setActiveTab('SHEET_METALS'); }} className="flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group" title="Bend Table">
        <div className="w-10 h-10 flex items-center justify-center text-sky-600 transition-transform group-hover:scale-110">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="3" y1="15" x2="21" y2="15"/><line x1="9" y1="3" x2="9" y2="21"/><line x1="15" y1="3" x2="15" y2="21"/></svg>
        </div>
        <span className="text-[10px] font-bold text-slate-800 leading-none uppercase">Bend<br/>Table</span>
      </button>
      <button onClick={() => { setActiveTab('SHEET_METALS'); setHint('Set global bend allowance parameters (K-Factor, relief type, bend radius) in the panel.'); }} className="flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group" title="Bend Allowance">
      ...
      <button onClick={() => { if (handleCreateFormingTool) { handleCreateFormingTool({ toolType: 'LANCE', width: 8, height: 3, depth: 4, radius: 1, angle: 60, thickness: 1.0, direction: 'OUTSIDE' }); } else { setHint('Select a face, then apply Lance forming tool.'); } setActiveTab('SHEET_METALS'); }} className="flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group" title="Lance">
        <div className="w-10 h-10 flex items-center justify-center text-emerald-600 transition-transform group-hover:scale-110">
      ...
      <button onClick={() => { if (handleCreateFormingTool) { handleCreateFormingTool({ toolType: 'BRIDGE', width: 12, height: 5, depth: 3, radius: 2, angle: 0, thickness: 1.0, direction: 'OUTSIDE' }); } else { setHint('Select a face, then apply Bridge forming tool.'); } setActiveTab('SHEET_METALS'); }} className="flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group" title="Bridge">
        <div className="w-10 h-10 flex items-center justify-center text-emerald-600 transition-transform group-hover:scale-110">
      ...
      <button onClick={() => { if (handleCreateFormingTool) { handleCreateFormingTool({ toolType: 'DIMPLE', width: 10, height: 2, depth: 2, radius: 3, angle: 0, thickness: 1.0, direction: 'OUTSIDE' }); } else { setHint('Select a face, then apply Dimple forming tool.'); } setActiveTab('SHEET_METALS'); }} className="flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group" title="Dimple">
        <div className="w-10 h-10 flex items-center justify-center text-emerald-600 transition-transform group-hover:scale-110">
      ...
      <button onClick={() => { if (handleCreateFormingTool) { handleCreateFormingTool({ toolType: 'DRAWN_CUTOUT', width: 8, height: 8, depth: 3, radius: 1, angle: 0, thickness: 1.0, direction: 'OUTSIDE' }); } else { setHint('Select a face, then apply Drawn Cutout forming tool.'); } setActiveTab('SHEET_METALS'); }} className="flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group" title="Drawn Cutout">
        <div className="w-10 h-10 flex items-center justify-center text-emerald-600 transition-transform group-hover:scale-110">
