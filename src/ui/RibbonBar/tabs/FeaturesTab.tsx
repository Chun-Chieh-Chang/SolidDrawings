...
export const FeaturesTab: React.FC<RibbonTabProps> = ({
  ...
  return (
    <div className="flex items-center gap-2 h-full animate-in fade-in slide-in-from-left-2 duration-300">
      ...
      <button onClick={() => { if (solidSketchPointCount >= 3) { handleExitAndExtrude?.('ADD'); } else { setSketchMode(true); setSketchTool('SELECT'); } }} className="flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[65px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group" title="Extrude Up To Next">
        <div className="w-10 h-10 flex items-center justify-center text-[#005B9A] transition-transform group-hover:scale-110">
      ...
      <button onClick={() => { addFeature({ id: `feat_${uuidv4()}`, type: 'HELICAL_SWEEP', name: `Helix ${features.filter(f => f.type === 'HELICAL_SWEEP').length + 1}`, parameters: { profile_id: '', pitch: 5, revolutions: 10, diameter: 20, handedness: 'CW', start_angle: 0, taper_angle: 0, axis_ref: null } }); setHint('Helical Sweep created. Configure in PropertyManager.'); }} className="flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group" title="Helical Sweep">
        <div className="w-10 h-10 flex items-center justify-center text-[#005B9A] transition-transform group-hover:scale-110">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 2C12 2 15 5 15 10C15 15 9 15 9 10C9 5 12 2 12 2Z" opacity="0.5"/><path d="M12 22V2M12 22C16 22 19 19 19 15C19 11 12 8 12 2M12 22C8 22 5 19 5 15C5 11 12 8 12 2"/></svg>
        </div>
      ...
      <button onClick={() => { const featId = `feat_${uuidv4()}`; addFeature({ id: featId, type: 'COMBINE', name: `Intersect ${features.filter(f => f.type === 'COMBINE' && f.parameters?.operation === 'INTERSECT').length + 1}`, parameters: { operation: 'INTERSECT', tool_feature_id: '' } }); setPendingFeatureCommand('COMBINE'); setHint('Intersect: select a tool body to intersect with the current part.'); }} className={`flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border ${pendingFeatureCommand === 'COMBINE' ? 'bg-white border-[#A0A0A0] shadow-inner' : 'border-transparent hover:bg-white hover:border-[#A0A0A0]'} active:bg-slate-100 group`} title="Intersect">
        <div className={`w-10 h-10 flex items-center justify-center transition-transform ${pendingFeatureCommand === 'COMBINE' ? 'text-orange-600 scale-110' : 'text-slate-600 group-hover:scale-110'}`}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="9" cy="12" r="6"/><circle cx="15" cy="12" r="6"/><path d="M9 6a6 6 0 0 0 0 12 6 6 0 0 0 6-6"/></svg>
        </div>
        <span className="text-[10px] font-bold text-slate-800 leading-none uppercase">Intersect</span>
      </button>
      <div className="w-[1px] h-10 bg-border/50 mx-1" />
      <button onClick={() => { const featId = `feat_${uuidv4()}`; addFeature({ id: featId, type: 'PATTERN', name: `Pattern ${features.filter(f => f.type === 'PATTERN').length + 1}`, parameters: { pattern_type: 'LINEAR', count: 2, spacing: 10, axis: 'X', direction_refs: [], target_feature_ids: [] } }); setSelectedId(featId); appliedEdgeFeatureRef.current = null; setActiveTab('FEATURES'); if (skipWizardIfRobotWorking()) { setPendingFeatureCommand('PATTERN'); setSelectedTopology(null); setHint('Select a target feature, then select a linear edge for direction.'); } }} className={`flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border ${pendingFeatureCommand === 'PATTERN' ? 'bg-white border-[#A0A0A0] shadow-inner' : 'border-transparent hover:bg-white hover:border-[#A0A0A0]'} active:bg-slate-100 group`} title="Pattern">
      ...
      <div className="w-[1px] h-10 bg-border/50 mx-1" />
      <button onClick={() => { const featId = `feat_${uuidv4()}`; addFeature({ id: featId, type: 'REFERENCE_PLANE', name: `Plane ${features.filter(f => f.type === 'REFERENCE_PLANE').length + 1}`, parameters: { planeType: 'OFFSET', offset: 10, refs: [], flip: false } }); setSelectedId(featId); setSelectedSubNodeType('FEATURE'); if (skipWizardIfRobotWorking()) setHint('Select entities (faces, points, edges) to define the reference plane.'); }} className={`flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group`} title="Reference Plane">
      ...
      <button onClick={() => { const featId = `feat_${uuidv4()}`; addFeature({ id: featId, type: 'REFERENCE_AXIS', name: `Axis ${features.filter(f => f.type === 'REFERENCE_AXIS').length + 1}`, parameters: { axisType: 'TWO_POINTS', refs: [] } }); setSelectedId(featId); setSelectedSubNodeType('FEATURE'); if (skipWizardIfRobotWorking()) setHint('Select entities to define the reference axis.'); }} className={`flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group`} title="Reference Axis">
        <div className={`w-10 h-10 flex items-center justify-center transition-transform text-slate-600 group-hover:scale-110`}>
      ...
      <button onClick={() => { const featId = `feat_${uuidv4()}`; addFeature({ id: featId, type: 'REFERENCE_POINT', name: `Point ${features.filter(f => f.type === 'REFERENCE_POINT').length + 1}`, parameters: { pointType: 'FACE_CENTER', refs: [] } }); setSelectedId(featId); setSelectedSubNodeType('FEATURE'); if (skipWizardIfRobotWorking()) setHint('Select entities to define the reference point.'); }} className={`flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group`} title="Reference Point">
        <div className={`w-10 h-10 flex items-center justify-center transition-transform text-slate-600 group-hover:scale-110`}>
