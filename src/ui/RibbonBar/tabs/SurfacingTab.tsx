...
export const SurfacingTab: React.FC<RibbonTabProps> = ({
  ...
  return (
    <div className="flex items-center gap-1 h-full animate-in fade-in slide-in-from-left-2 duration-300">
      ...
      <button onClick={() => { const featId = `feat_${uuidv4()}`; addFeature({ id: featId, type: 'LOFT', name: `Surface-Loft ${features.filter(f => f.type === 'LOFT').length + 1}`, parameters: { profile_ids: [], isSurfaceOnly: true } }); }} className="flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group" title="Lofted Surface">
        <div className="w-10 h-10 flex items-center justify-center text-orange-500 transition-transform group-hover:scale-110">
      ...
      <button onClick={() => { const featId = `feat_${uuidv4()}`; addFeature({ id: featId, type: 'SURFACE_OFFSET', name: `Offset-Surf ${features.filter(f => f.type === 'SURFACE_OFFSET').length + 1}`, parameters: { distance: 1.0, refs: [] } }); setPendingFeatureCommand('SURFACE_OFFSET'); if (skipWizardIfRobotWorking()) setHint('Select faces to offset as a surface.'); }} className="flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group" title="Offset Surface">
      ...
      <button onClick={() => { const featId = `feat_${uuidv4()}`; addFeature({ id: featId, type: 'SURFACE_KNIT', name: `Knit-Surf ${features.filter(f => f.type === 'SURFACE_KNIT').length + 1}`, parameters: { refs: [] } }); setPendingFeatureCommand('SURFACE_KNIT'); setHint('Knit surface feature created. Select surfaces to knit.'); }} className="flex flex-col items-center justify-center gap-0.5 px-3 h-[78px] min-w-[75px] transition-all border border-transparent hover:bg-white hover:border-[#A0A0A0] active:bg-slate-100 group" title="Knit Surface">
